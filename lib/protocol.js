...
export const MESSAGE_PING_PONG = 5 // [5, 0] is a ping. [5, 1] is a pong

export const encodePingMessage = () => {
  const encoder = encoding.createEncoder()
  encoding.writeVarUint(encoder, MESSAGE_PING_PONG)
  encoding.writeVarUint(encoder, 0)
  return encoding.toUint8Array(encoder)
}

export const encodePongMessage = () => {
  const encoder = encoding.createEncoder()
  encoding.writeVarUint(encoder, MESSAGE_PING_PONG)
...
export const clientReadMessage = (decoder, provider) => {
  ...
  while (decoding.hasContent(decoder)) {
    switch (decoding.readVarUint(decoder)) {
      ...
      case MESSAGE_PING_PONG: {
        const isPing = decoding.readVarUint(decoder) === 0
        if (isPing) {
          provider._send(encodePongMessage())
        }
      }
    }
  }
  ...
  while (decoding.hasContent(decoder)) {
    switch (decoding.readUint8(decoder)) {
      ...
      case MESSAGE_PING_PONG: {
        const isPing = decoding.readVarUint(decoder) === 0
        if (isPing) {
          clientConn._send(encodePongMessage())
        }
